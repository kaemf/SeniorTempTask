...
One deliberate asymmetry worth naming: a confirmed approval keeps `proposedById` on the final row as provenance, while a rejection clears it (the proposal is no longer active; the proposer remains reconstructable from the audit trail). Combined with the `RESTRICT` FK this means users who proposed a still-approved loan cannot be deleted, which is the intended bias: financial provenance outlives convenience.
